...
game_pairs <- game_stats |>
  ...
  dplyr::mutate(
    ...
    pace_40   = game_poss / game_minutes * 40
  )

message("Game-level stats calculated. Games: ", nrow(game_pairs))

iterate_ratings <- function(season_games, n_iter = 30) {
  league_avg <- mean(season_games$raw_ortg, na.rm = TRUE)
  
  ratings <- season_games |>
    dplyr::group_by(team_id) |>
    dplyr::summarise(
      ortg = mean(raw_ortg, na.rm = TRUE),
      drtg = mean(raw_drtg, na.rm = TRUE),
      .groups = "drop"
    )
  ...
  for (i in seq_len(n_iter)) {
    ...
    new_ratings <- season_games |>
      ...
      dplyr::group_by(team_id) |>
      dplyr::summarise(
        ortg = mean(raw_ortg + (league_avg - opp_drtg), na.rm = TRUE),
        drtg = mean(raw_drtg + (league_avg - opp_ortg), na.rm = TRUE),
        .groups = "drop"
      )
...
message("Running iterative adjustments...")
