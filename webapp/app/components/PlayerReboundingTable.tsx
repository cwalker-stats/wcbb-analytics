...
const REBOUNDING_COLUMNS: ColumnDef[] = [
    { key: "orb_pct", label: "ORB%", ascending: false, decimals: 1, tooltip: "Percentage of available offensive rebounds secured." },
    { key: "drb_pct", label: "DRB%", ascending: false, decimals: 1, tooltip: "Percentage of available defensive rebounds secured." },
    { key: "trb_pct", label: "TRB%", ascending: false, decimals: 1, tooltip: "Percentage of all available rebounds secured." },
];

const DEFENSE_COLUMNS: ColumnDef[] = [
    { key: "stl_pct", label: "STL%", ascending: false, decimals: 1, tooltip: "Percentage of opponent possessions resulting in a steal." },
    { key: "blk_pct", label: "BLK%", ascending: false, decimals: 1, tooltip: "Percentage of opponent shots blocked." },
    { key: "stocks_40", label: "Stocks", ascending: false, decimals: 2, tooltip: "Steals plus blocks per 40 minutes." },
];

const ALL_COLUMNS = [...REBOUNDING_COLUMNS, ...DEFENSE_COLUMNS];
...
export default function PlayerReboundingTable() {
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.875rem" }}>
                    <thead>
                        <tr>
                            ...
                            <th colSpan={3} style={{ ...headerStyle, backgroundColor: "rgba(196,92,92,0.06)", borderRight: "2px solid var(--border)" }}>REBOUNDING</th>
                            <th colSpan={3} style={{ ...headerStyle, backgroundColor: "rgba(196,92,92,0.04)" }}>DEFENSE</th>
                        </tr>
                        ...
                        {paginated.map((player, i: number) => (
                            <tr key={`${player.athlete_id}-${player.season}`} style={{ borderBottom: "1px solid var(--border)", backgroundColor: i % 2 === 0 ? "transparent" : "var(--bg-card)" }}>
                                ...
                                {ALL_COLUMNS.map((col: ColumnDef, ci: number) => {
                                    ...
                                    const isGroupBoundary = ci === REBOUNDING_COLUMNS.length - 1;
                                    return (
