...
const CREATION_COLUMNS: ColumnDef[] = [
    { key: "off_load", label: "OffLoad", ascending: false, decimals: 1, pct: true, tooltip: "Share of possessions involving a shot, assist, or turnover." },
    { key: "ast_pct", label: "AST%", ascending: false, decimals: 1, tooltip: "Share of teammate field goals assisted while on the floor." },
    { key: "net_play", label: "NetPlay", ascending: false, decimals: 1, showPlus: true, tooltip: "AST% minus TOV%." },
    { key: "ast_to", label: "A/TO", ascending: false, decimals: 2, tooltip: "Assists per turnover." },
];

const SECURITY_COLUMNS: ColumnDef[] = [
    { key: "tov_pct", label: "TOV%", ascending: true, decimals: 1, tooltip: "Percentage of possessions that end in a turnover." },
    { key: "to_economy", label: "TOEcon", ascending: false, decimals: 3, tooltip: "Assists as a share of assists and turnovers." },
    { key: "ppr", label: "PPR", ascending: false, decimals: 2, showPlus: true, tooltip: "Playmaking rating based on assist and turnover volume." },
];

const ALL_COLUMNS = [...CREATION_COLUMNS, ...SECURITY_COLUMNS];
...
export default function PlayerPlaymakingTable() {
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.875rem" }}>
                    <thead>
                        <tr>
                            ...
                            <th colSpan={4} style={{ ...headerStyle, backgroundColor: "rgba(74,158,107,0.06)", borderRight: "2px solid var(--border)" }}>CREATION</th>
                            <th colSpan={3} style={{ ...headerStyle, backgroundColor: "rgba(74,158,107,0.04)" }}>BALL SECURITY</th>
                        </tr>
                        ...
                        {paginated.map((player, i: number) => (
                            <tr key={`${player.athlete_id}-${player.season}`} style={{ borderBottom: "1px solid var(--border)", backgroundColor: i % 2 === 0 ? "transparent" : "var(--bg-card)" }}>
                                ...
                                {ALL_COLUMNS.map((col: ColumnDef, ci: number) => {
                                    ...
                                    const isGroupBoundary = ci === CREATION_COLUMNS.length - 1;
                                    return (
